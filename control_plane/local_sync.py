...
This bridge intentionally excludes audit history, financial ledger contents,
credentials, destinations, and other sensitive runtime details. It exists so
remote tooling can inspect the current local operational state without asking
Boss to copy terminal output into chat.
"""
...
def build_snapshot() -> dict:
    ...
    return {
        ...
        "source": "local-leverage-runtime",
        "privacy": "sanitized-no-finance-no-audit",
        "readiness": {
            "ready": readiness["ready"],
            "status": readiness["status"],
            "release_gate": readiness["release_gate"],
            "checks": [{"name": c["name"], "status": c["status"]} for c in readiness["checks"]],
        },
        "health": {
            "status": health.get("status"),
            "alert_count": health.get("summary", {}).get("total", 0),
        },
        "company": {
...
def sync(push: bool = True) -> int:
    if push:
        ...
        if status.returncode != 0:
            print(status.stderr.strip() or "git status failed")
            return 2
        allowed = {"dashboard/local_state.json"}
        ...
        if pull.returncode != 0:
            print("SYNC BLOCKED: git pull failed; local changes were not overwritten.")
    ...
    if not push:
        print(f"Local state written: {OUTPUT}")
        return 0
    ...
    if diff.returncode == 0:
        print("LEVERAGE LOCAL SYNC: no state change")
        return 0
    commit = _run("git", "commit", "-m", "chore: sync sanitized local company state")
    if commit.returncode != 0:
        print(commit.stderr.strip() or "git commit failed")
        return 2
    push_result = _run("git", "push")
    if push_result.returncode != 0:
        print("SYNC BLOCKED: git push failed. Local commit is preserved for inspection.")
        print(push_result.stderr.strip())
        return 2
    print("LEVERAGE LOCAL SYNC: published sanitized local state")
    return 0
