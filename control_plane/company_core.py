...
def validate_project(project: Project) -> list[str]:
    ...
    if project.lifecycle_stage not in LIFECYCLE:
        errors.append(f"invalid lifecycle stage: {project.lifecycle_stage}")
    if project.status not in LIFECYCLE:
        errors.append(f"invalid project status: {project.status}")
    if project.capital_deployed < 0:
        errors.append("capital_deployed cannot be negative")
    if project.currency != "MYR":
        errors.append("default company currency is MYR")
    return errors
...
def change_stage(project_id: str, stage: str, reason: str) -> Project:
    ...
    for item in data.get("projects", []):
        if item["id"] == project_id:
            old = item.get("lifecycle_stage", "intake")
...
def approval_required(action: str) -> bool:
    restricted = {
        "move_money", "approve_money", "open_financial_account",
        "change_bank_details", "publish_irreversible_contract",
        "delete_production_data", "deploy_unreviewed_external_change",
    }
...
if __name__ == "__main__":
    company = load_json(COMPANY_FILE)
    projects = list_projects()
    print(f"Leverage company: {company['company']['name']}")
    print(f"Projects registered: {len(projects)}")
    for project in projects:
        print(f"- {project.id}: {project.lifecycle_stage}")
