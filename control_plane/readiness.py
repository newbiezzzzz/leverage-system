...
This module answers one question: is the company operating system solid enough
that we can start building the next real income project on top of it?

It never mutates company state and never enables money movement.
"""
...
EXPECTED_WORKERS = {
    ...
    "data-worker": "validate",
    "code-worker": "build",
    "project-manager": "plan",
    "operations-worker": "verify",
    "customer-worker": "intake",
...
def company_os_readiness() -> dict:
    ...
    try:
        ...
        missing_workers = [worker_id for worker_id in EXPECTED_WORKERS if worker_id not in by_id]
        offline = [worker_id for worker_id, worker in by_id.items() if worker_id in EXPECTED_WORKERS and worker.get("status") != "online"]
        unverified = [worker_id for worker_id, worker in by_id.items() if worker_id in EXPECTED_WORKERS and worker.get("activation") != "verified"]
        bad_capabilities = [worker_id for worker_id, action in EXPECTED_WORKERS.items() if worker_id in by_id and action not in set(by_id[worker_id].get("capabilities", []))]
        ...
        if not worker_ok:
            problems = missing_workers + offline + unverified + bad_capabilities
            detail = f"Worker contract problems: {', '.join(sorted(set(problems)))}"
    ...
    try:
        ...
        policy_ok = (
            policy.get("default_mode") == "owner-controlled"
            and policy.get("spend_requires_approval") is True
            ...
            and policy.get("unknown_provider_state") == "safe_mode"
            and policy.get("unknown_balance_state") == "blocked"
    ...
    try:
        ...
        money_policy = ledger.get("policy", {})
        money_ok = (
            money_policy.get("live_money_movement") is False
            and money_policy.get("worker_can_prepare_payout") is True
            and money_policy.get("worker_can_approve_payout") is False
            and money_policy.get("worker_can_execute_payout") is False
            and money_policy.get("owner_approval_required") is True
            and money_policy.get("never_assume_balance") is True
    ...
    try:
        resource_limits = _load(ROOT / "resource_limits.json")
        resources = resource_limits.get("resources", [])
